<script lang="ts">
	...
	import RemoveMeiliButton from '../lib/RemoveMeiliButton.svelte';
	...
	let search = async () => {
		let path =
			$api + '/search/' + $category + '/' + encodeURIComponent(encodeURIComponent(input_field));
	...
	let addItem = async (item_to_add: string) => {
		...
		input_field = '';
		$matches = [];
		item_to_add = item_to_add[0].toUpperCase() + item_to_add.slice(1).toLowerCase();
		let found = false;
