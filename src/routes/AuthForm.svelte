...
<div class="flex min-h-screen items-center justify-center p-4">
    <div
        class="w-full max-w-md rounded-lg bg-primary-900 bg-opacity-90 p-8 shadow-lg"
    >
        ...
        <form method="POST" class="space-y-4" use:enhance>
            ...
            <div>
                ...
                <input
                    ...
                    minlength="3"
                    value={username}
                    class="w-full rounded-md border border-primary-900 bg-black-bean text-thistle shadow-sm focus:border-plum focus:ring-plum sm:text-sm placeholder-gray-500
                    h-8 p-2"
                    placeholder={isLogin
                        ? "Enter username"
                        : "Choose a username"}
            ...
            <div>
                ...
                <input
                    ...
                    required
                    minlength="6"
                    class="w-full rounded-md border border-primary-900 bg-black-bean text-thistle shadow-sm focus:border-plum focus:ring-plum sm:text-sm placeholder-gray-500 h-8 p-2"
                    placeholder={isLogin
                        ? "Enter password"
                        : "Create a password"}
                />
            ...
            <button
                type="submit"
                class="w-full rounded-md bg-primary-800 py-2 px-4 text-thistle hover:bg-primary-900/90 focus:outline-none focus:ring-2 focus:ring-primary-900 focus:ring-offset-2 focus:ring-offset-seal-brown"
            >
                {buttonLabel}
            </button>
        </form>
        ...
        <p class="mt-6 text-center text-sm text-plum">
            {altPrompt}
            <a href={altLink} class="font-medium text-gray-900 hover:text-plum">
                {altText}
            </a>
        </p>
    </div>
